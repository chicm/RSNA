...
from lovasz_losses import lovasz_loss_ignore_empty, lovasz_hinge
...
class UNet(nn.Module):
    def __init__(self, basenet='senet154', num_filters=16, pretrained='imagenet'):
        super().__init__()
        net, bn, n_pretrained = create_basenet(basenet, pretrained)
        self.name = 'UNet_'+basenet
    ...
    def forward(self, x):
        ...
        logit = self.logit(torch.cat([  # fuse
            fuse_pixel,
            F.upsample(fuse_image.view(batch_size, -1, 1, 1, ), scale_factor=256, mode='nearest')
        ], 1))
        #print(logit.size(), logit_image.size())
...
if __name__ == '__main__':
    ...
    parser.add_argument("--basenet", choices=BASENET_CHOICES, default='resnet34', help='model of basenet')
    ...
    x = torch.empty((8, 3, 256, 256))
    logit, logit_pixel, logit_image = net(x)
    print(x.size(), logit.size(), logit_image.size())
